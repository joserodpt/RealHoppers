...
MODULE_DIR="$PROJECT_DIR/realhoppers-plugin"
...
if [[ -z "$VERSION" ]]; then
  echo "Error: Could not read plugin version from realhoppers-plugin/pom.xml"
  exit 1
fi

JAR_NAME="realhoppers-${VERSION}.jar"
OUTPUT_JAR_NAME="RealHoppers-${VERSION}.jar"

echo "Building RealHoppers..."
mvn -pl realhoppers-plugin -am clean install

SOURCE_JAR="$MODULE_DIR/target/$JAR_NAME"
if [[ ! -f "$SOURCE_JAR" ]]; then
  SOURCE_JAR="$(ls -t "$MODULE_DIR"/target/realhoppers-*.jar 2>/dev/null | grep -v '/original-' | head -n 1 || true)"
